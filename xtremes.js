    ...
    try {
        ...
        db.run(
            ...
            (err) => {
                ...
                // Return success + redirect instruction
                res.json({ success: true, userId, redirectTo: 'login' });
            }
        );
    } catch (e) {
...
app.post('/api/withdraw', (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE id = ?", [userId], (err, user) => {
        ...
        if (user.balance < amount) {
            return res.status(400).json({ error: "Liquidity error" });
        }

        console.log("Sending withdrawal email for user:", user.username, "amount:", amount);
...
app.post('/api/admin/update-user', (req, res) => {
    ...
    // Check if we are updating the password or keeping the old one
    if (password) {
